...
Sie können mit der Planner-API in Microsoft Graph Aufgaben erstellen und diese Benutzern in einer Gruppe in Office 365 zuweisen.

Bevor Sie mit der Arbeit in der Planner-API beginnen, sollten Sie wissen, wie die wichtigsten Objekte zueinander und zu Office 365-Gruppen in Beziehung stehen.

## <a name="groups"></a>Gruppen

Office 365-Gruppen sind die Besitzer der Pläne in der Planner-API.
Zum Abrufen der Pläne [im Besitz einer Gruppe stellen Sie](../api/plannergroup_list_plans.md)die folgenden HTTP-Anforderung.

``` http
GET /groups/{id}/planner/plans
```

Wenn Sie [einen neuen Plan erstellen](../api/planner_post_plans.md), definieren Sie eine Gruppe als dessen Besitzer, indem Sie einfach die `owner`-Eigenschaft für ein Planobjekt festlegen. Pläne müssen im Besitz von Gruppen sein.

>**Hinweis:** Der Benutzer, der den Plan erstellt, muss ein Mitglied der Besitzergruppe sein. Wenn Sie mit [Gruppe erstellen](../api/group_post_groups.md) eine neue Gruppe erstellen, werden Sie nicht als Mitglied der Gruppe hinzugefügt. Nach dem Erstellen der Gruppe fügen Sie sich selbst mithilfe der [Gruppe Beitragsmitglieder](../api/group_post_members.md)als Mitglied hinzu.

## <a name="plans"></a>Pläne

[Pläne](plannerplan.md) sind die Container von [Aufgaben](plannertask.md). Setzen Sie zum [Erstellen einer Aufgabe in einem Plan](../api/planner_post_tasks.md) die `planId`-Eigenschaft für das Aufgaben-Objekt auf die ID des Plans während des Erstellens der Aufgabe.
Aufgaben können derzeit nicht ohne Pläne erstellt werden.
Stellen Sie zum [Abrufen der Aufgaben in einem Plan](../api/plannerplan_list_tasks.md)die folgende HTTP-Anforderung.
