...
Get neu erstellt, aktualisiert oder gelöscht Directory Rollen ohne ein alles Lesen der gesamte Ressource Auflistung ausführen. Einzelheiten finden Sie unter [Delta-Abfrage verwenden](../../../concepts/delta_query_overview.md) .
...
Zum Nachverfolgen von Änderungen beginnen, stellen Sie eine Anforderung, einschließlich der **Delta** -Funktion für die Ressource [DirectoryRole](../resources/directoryrole.md) .
...
|:---------------|:--------|:----------|
| $deltatoken | string | Ein [Token Zustand](../../../concepts/delta_query_overview.md) zurückgegeben, die der `deltaLink` URL des vorherigen **Delta** Funktionsaufrufs für dieselbe Ressource-Auflistung zurück, der angibt, Rundung des Änderungsprotokolls nach Abschluss. Speichern und übernehmen Sie die gesamte `deltaLink` URL einschließlich dieses Token in die erste Anforderung der nächsten der Versionsvergleich für diese Auflistung.|
| $skiptoken | string | Ein [Token Zustand](../../../concepts/delta_query_overview.md) zurückgegeben, die der `nextLink` URL von der vorherigen **Delta** -Funktionsaufruf, der angibt, es sind weitere Änderungen in der gleichen Auflistung Ressource überwacht werden müssen. |

### <a name="odata-query-parameters"></a>OData-Abfrageparameter

Diese Methode unterstützt OData-Abfrage-Parameter, mit denen die Antwort anpassen.
...
Wenn der Vorgang erfolgreich war, gibt diese Methode `200 OK` Antwort Code und [DirectoryRole](../resources/directoryrole.md) -Auflistungsobjekt in der Antworttext. Die Antwort enthält außerdem eine `nextLink` URL oder eine `deltaLink` URL.
...
- Wenn ein `deltaLink` URL wird zurückgegeben, es sind keine weiteren Daten zum vorhandenen Status der Ressource zurückgegeben werden soll. Speichern Sie `deltaLink` URL, und wenden Sie es im nächsten **Delta** Aufruf über Änderungen an der Ressource in der Zukunft zu informieren.
...
- [Verwendung Delta-Abfrage zum Nachverfolgen von Änderungen in Microsoft Graph-Daten](../../../concepts/delta_query_overview.md) für weitere details
- Beispielanforderungen finden Sie unter [Inkrementelle Änderungen für Benutzer abrufen](../../../concepts/delta_query_users.md).
