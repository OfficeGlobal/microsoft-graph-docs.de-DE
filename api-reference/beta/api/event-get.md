...
Das zweite Beispiel zeigt, wie Sie eine `Prefer: outlook.body-content-type="text"` Header, um die **Body** -Eigenschaft des angegebenen Ereignisses im Textformat erhalten möchten.
...
Im dritte Beispiel zeigt ein Ereignis, das mehr als einem Standort gibt abrufen. Die Anforderung gibt einen `$select`-Abfrageparameter zum Zurückgeben bestimmter Eigenschaften an. 
...
Da die Anforderung keine festlegt `Prefer: outlook.timezone` oder `Prefer: outlook.body-content-type` Header, den **start** und **End** -Eigenschaften werden in der Standardeinstellung UTC-Zeitzone angezeigt, und der Textkörper weist das Standard-HTML-Format.  
