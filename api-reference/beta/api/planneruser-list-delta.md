...
Ruft die Änderungen für Objekte, denen der Benutzer [angemeldet](../resources/planner-overview.md#track-changes-using-delta-query) ist.

Mit dieser Methode können die Anwendung zum Nachverfolgen von Änderungen auf Objekte, die der Benutzer über einen Zeitraum von in Planner zugreifen kann.

Der Rückgabewert dieser Methode kann heterogenes Typen von Objekten aus Planner enthalten.

Weitere Informationen zum Nachverfolgen von Änderungen in Microsoft Graph-Daten finden Sie unter [Verwendung Delta-Abfrage zum Nachverfolgen von Änderungen in Microsoft Graph-Daten](/graph/delta-query-overview).
...
Keine zusätzlichen Abfrageparametern (z. B. `$select`, `$expand`, oder `$filter`) werden derzeit Planner Implementierung der Delta-Abfragen unterstützt.
...
Wenn der Vorgang erfolgreich war, gibt diese Methode einen `200 OK` Antwortcode und eine Auflistung von Änderungen auf Objekte in der Antworttext und einen Link Delta Sync, denen Sie folgen angewendet werden soll.

Wenn die `deltaLink` , dass der Anrufer verwendet ist falsch formatiert, diesen Endpunkt HTTP 400 zurück.

Wenn die `deltaLink` , dass der Anrufer verwendet ist veraltet, diesen Endpunkt HTTP 410 zurück.
...
>**Hinweis:** Das gezeigte Antwortobjekt kann zur besseren Lesbarkeit gekürzt werden. Alle geänderten Eigenschaften werden von einem tatsächlichen Aufruf zurückgegeben.
