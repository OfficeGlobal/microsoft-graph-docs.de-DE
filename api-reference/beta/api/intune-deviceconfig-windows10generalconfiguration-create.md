...
Erstellt ein neues [windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md)-Objekt.
...
|taskManagerBlockEndTask|Boolescher Wert|Geben Sie an, ob nicht-Administratoren Aufgaben mithilfe des Task-Managers beenden können.|
...
|enableAutomaticRedeployment|Boolescher Wert|Benutzern mit Administratorrechten das Löschen aller Benutzerdaten und-Einstellungen mithilfe von STRG + Win + R auf dem Bildschirm "Gerätesperre" geStatten, damit das Gerät automatisch neu konfiguriert und für die Verwaltung erneut registriert werden kann.|
...
|authenticationAllowSecondaryDevice|Boolescher Wert|Ermöglicht die Verwendung sekundärer Authentifizierungsgeräte mit Windows.|
...
|authenticationPreferredAzureADTenantDomainName|String|Gibt die bevorzugte Domäne unter den verfügbaren Domänen im Azure AD-Mandanten an.|
|cryptographyAllowFipsAlgorithmPolicy|Boolescher Wert|Geben Sie an, ob die FIPS-Richtlinie (Federal Information Processing Standard) zugelassen oder verweigert werden soll.|
...
|messagingBlockSync|Boolescher Wert|Gibt an, ob das Sichern und Wiederherstellen und Nachrichten von Textnachrichten überall blockiert werden soll.|
|messagingBlockMMS|Boolescher Wert|Gibt an, ob die MMS-Sende-/Empfangsfunktionalität auf dem Gerät blockiert werden soll.|
|messagingBlockRichCommunicationServices|Boolescher Wert|Gibt an, ob die RCS-Sende-und-Empfangsfunktionalität auf dem Gerät blockiert werden soll.|
...
|printerDefaultName|String|Name (Netzwerk Hostname) eines installierten Druckers.|
|printerBlockAddition|Boolescher Wert|Verhindern der Benutzerinstallation zusätzlicher Drucker aus den Druckereinstellungen.|
...
|searchDisableUseLocation|Boolescher Wert|Gibt an, ob die Suche Standortinformationen verwenden kann.|
|searchDisableLocation|Boolescher Wert|Gibt an, ob die Suche Standortinformationen verwenden kann.|
...
|searchBlockWebResults|Boolescher Wert|Gibt an, ob die Websuche blockiert werden soll.|
|securityBlockAzureADJoinedDevicesAutoEncryption|Boolescher Wert|Geben Sie an, ob die automatische Geräteverschlüsselung während OOBE zugelassen werden soll, wenn das Gerät mit Azure AD verbunden ist (nur Desktop).|
...
|systemTelemetryProxyServer|String|Ruft den vollqualifizierten Domänennamen (FQDN) oder die IP-Adresse eines Proxyservers ab, um verbundene Benutzererlebnisse und teleMetrie-Anforderungen weiterzuleiten, oder legt diesen fest.|
...
|inkWorkspaceBlockSuggestedApps|Boolescher Wert|Geben Sie an, ob Empfohlene App-Vorschläge im frei Hand Arbeitsbereich angezeigt werden sollen.|
...
|bluetoothBlockPromptedProximalConnections|Boolescher Wert|Gibt an, ob die Benutzer mit SWIFT-Paaren und anderen Näherungs basierten Szenarien blockiert werden sollen.|
...
|edgeFavoritesListLocation|String|Der Speicherort der Favoritenliste, die festgestellt werden soll. Dies kann eine lokale Datei, ein lokales Netzwerk oder ein http-Speicherort sein.|
|edgeBlockEditFavorites|Boolescher Wert|Gibt an, ob verhindert werden soll, dass der Benutzer Änderungen an den Favoriten vornimmt.|
|edgeNewTabPageURL|String|Geben Sie die Seite an, die beim Erstellen neuer Registerkarten geöffnet wird.|
...
|edgeHomeButtonConfigurationEnabled|Boolescher Wert|Aktivieren Sie die Schaltfläche Konfiguration der Startseite.|
...
|edgeBlockSideloadingExtensions|Boolescher Wert|Gibt an, ob der Benutzer Erweiterungen querladen kann.|
...
|edgeBlockPrinting|Boolescher Wert|Konfigurieren von Edge zum Zulassen oder Blockieren des Druckens.|
...
|edgeBlockSavingHistory|Boolescher Wert|Konfigurieren Sie Edge so, dass der Browserverlauf gespeichert oder der Browserverlauf nie gespeichert wird.|
|edgeBlockFullScreenMode|Boolescher Wert|Zulassen oder verhindern, dass Edge in den Vollbildmodus wechselt.|
|edgeBlockWebContentOnNewTabPage|Boolescher Wert|Konfigurieren Sie, um eine leere Seite in Edge anstelle der standardmäßigen neuen Registerkarte zu laden und zu verhindern, dass Benutzer Sie ändern.|
|edgeBlockTabPreloading|Boolescher Wert|Konfigurieren Sie, ob Edge die neue Registerkarte beim Windows-Start vorlädt.|
|edgeBlockPrelaunch|Boolescher Wert|Entscheiden Sie, ob Microsoft Edge beim Windows-Start vorgestartet wird.|
...
|edgePreventCertificateErrorOverride|Boolescher Wert|Zulassen oder verhindern, dass Benutzerzertifikat Fehler überschreiben.|
...
|defenderScheduleScanEnableLowCpuPriority|Boolescher Wert|Wenn diese Option aktiviert ist, wird bei geplanten Scans eine niedrige CPU-Priorität verwendet.|
|defenderDisableCatchupQuickScan|Boolescher Wert|Nach der Blockierung werden die Catch-up-Scans für geplante Schnellscans deaktiviert.|
|defenderDisableCatchupFullScan|Boolescher Wert|Nach der Blockierung werden die Catch-up-Scans für geplante vollständige Scans deaktiviert.|
...
|defenderBlockOnAccessProtection|Boolescher Wert|Ermöglicht oder deaktiviert Windows Defender bei Zugriffsschutzfunktionen.|
...
|privacyDisableLaunchExperience|Boolescher Wert|Diese Richtlinie verhindert, dass die Datenschutzumgebung während der Benutzeranmeldung für neue und aktualisierte Benutzer gestartet wird.|
...
|privacyBlockPublishUserActivities|Boolescher Wert|Blockiert die gemeinsamen Erfahrungen/Ermittlung von zuletzt verwendeten Ressourcen in Task Switcher usw.|
|privacyBlockActivityFeed|Boolescher Wert|Blockiert die Verwendung von Cloud-basierten Sprachdiensten für Cortana-, Diktat-oder Store-Anwendungen.|
...
|edgeBlockSearchSuggestions|Boolescher Wert|Gibt an, ob die Verwendung der Suchvorschläge in der Adressleiste verhindert werden soll.|
|edgeBlockSearchEngineCustomization|Boolescher Wert|Gibt an, ob verhindert werden soll, dass der Benutzer neue Suchmaschine hinzufügt oder die Standardsuchmaschine ändert.|
|edgeBlockSendingIntranetTrafficToInternetExplorer|Boolescher Wert|Gibt an, ob der Intranet-Datenverkehr von Edge zu Internet Explorer gewechselt werden soll. Hinweis: der Name dieser Eigenschaft ist irreführend; die Eigenschaft ist veraltet, verwenden Sie stattdessen EdgeSendIntranetTrafficToInternetExplorer.|
|edgeSendIntranetTrafficToInternetExplorer|Boolescher Wert|Gibt an, ob der Intranet-Datenverkehr von Edge zu Internet Explorer gewechselt werden soll.|
...
|tenantLockdownRequireNetworkDuringOutOfBoxExperience|Boolescher Wert|Gibt an, ob das Gerät zum Herstellen einer Verbindung mit dem Netzwerk erforderlich ist.|
|appManagementMSIAllowUserControlOverInstall|Boolescher Wert|Mit dieser Richtlinieneinstellung können Benutzer Installationsoptionen ändern, die normalerweise nur Systemadministratoren zur Verfügung stehen.|
|appManagementMSIAlwaysInstallWithElevatedPrivileges|Boolescher Wert|Diese Richtlinieneinstellung weist Windows Installer an, bei der Installation von Programmen auf dem System erhöhte Berechtigungen zu verwenden.|
|dataProtectionBlockDirectMemoryAccess|Boolescher Wert|Mit dieser Richtlinieneinstellung können Sie DMA (Direct Memory Access) für alle Hot-Plug-fähigen PCI Downstream-Ports blockieren, bis sich ein Benutzer an Windows anmeldet.|
...
Bei erfolgreicher Ausführung gibt die Methode den Antwortcode `201 Created` und ein Objekt des Typs [windows10GeneralConfiguration](../resources/intune-deviceconfig-windows10generalconfiguration.md) im Antworttext zurück.
