...
5. **Wenn Sie das Microsoft Graph-SDK verwenden**, installieren und verweisen Sie auf das SDK.  
   a. Führen Sie an der Eingabeaufforderung den folgenden Befehl aus, um das SDK zu installieren.
        
        bower install https://github.com/microsoftgraph/msgraph-sdk-javascript.git
         
   b. Fügen Sie in der Datei **public/index.html** den folgenden Code zu dem **head**-Element über dem `<!-- App code. -->`-Abschnitt hinzu:
...
1. Öffnen Sie „aad.js“, und fügen Sie den folgenden Code hinzu. Dadurch wird die Kommunikation mit dem Azure AD-Authentifizierungsanbieter konfiguriert und ein Listener hinzugefügt, der die Authentifizierungsantwort speichert, die das Zugriffstoken enthält. (Die Skriptverweise zu HelloJS wurden bereits zur Ansicht „index.html“ hinzugefügt.)
...
3. Ersetzen Sie *//Den Benutzer anmelden und abmelden* durch den folgenden Code. Die Funktion **Anmelden** verwendet HelloJS, um Tokeninformationen zu erhalten. Der Listener in „aad.js“ speichert diese Informationen, einschließlich des Zugriffstokens, im lokalen Speicher.
...
4. **Wenn Sie das Microsoft Graph-SDK verwenden** öffnen Sie „app.js“ zu öffnen, und fügen Sie den folgenden Code am Ende der Datei hinzu. Dadurch wird das SDK initialisiert.
...
Wenn Sie das Microsoft Graph-SDK verwenden, lesen Sie weiter. Wenn Sie REST verwenden, lesen Sie den Abschnitt [Verwenden der REST-API](#using-the-rest-api).
...
6. Speichern Sie all Ihre Änderungen. Jetzt können Sie [die App ausführen](#run-the-app).
