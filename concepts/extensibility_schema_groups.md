# <a name="add-custom-data-to-groups-using-schema-extensions-preview"></a>Hinzufügen von benutzerdefinierten Daten zu Benutzern mithilfe von Schemaerweiterungen (Preview)
...
4. Hinzufügen oder Aktualisieren von benutzerdefinierten Daten zu einer vorhandenen Gruppe basierend auf einer Schemaerweiterungsdefinition
5. Einlesen einer Gruppe und der Erweiterungsdaten

>**Hinweis:** In diesem Artikel erfahren Sie, wie Sie Schemaerweiterungswerte in einer **group**-Ressource erstellen und lesen können (Schritte 3 bis 5).  Diese Methoden werden auch von Ressourcen des Typs **device**, **event**, **message**, **post** und **user** unterstützt.  Sie können also auf jede dieser Ressourcen ähnliche Vorgänge wie die unten beschriebenen Beispielanforderungen anwenden.
...
Erstellen Sie unter Verwendung der `graphlearn_courses`-Schemaerweiterungsdefinition, die wir soeben registriert haben, eine neue Gruppe, die mit zusätzlichen Daten erweitert wurde.  Dies ist ein standardmäßiger ```POST``` für die **Gruppenressource**, wobei die zusätzliche Erweiterung des komplexen Typs `graphlearn_courses` im Anforderungstext definiert ist.  Die Antwort spiegelt keine Datenerweiterungen wider. Für die Erweiterung muss explizit ```$select``` anhand des Namens unter Verwendung eines ```GET```-Vorgangs durchgeführt werden.
...
## <a name="4-add-or-update-custom-data-to-an-existing-group"></a>4. Hinzufügen oder Aktualisieren von benutzerdefinierten Daten zu einer vorhandenen Gruppe
Ähnlich wie im letzten Beispiel können wir eine vorhandenen Gruppenressource mit der zusätzlichen Erweiterung für den komplexen Typ `graphlearn_courses` erweitern, der im Textkörper einer ```PATCH```-Anforderung definiert ist.  
...
Um die Gruppe **und** ihre Erweiterungsdaten abzurufen, müssen wir `$select` verwenden, um die Erweiterung anhand des Namens anzugeben, in diesem Fall anhand von `graphlearn_courses`.
...
- [schemaExtension-Ressourcentyp](../api-reference/beta/resources/schemaextension.md)
- [schemaExtensions aufführen](../api-reference/beta/api/schemaextension_list.md)
- [schemaExtension erstellen](../api-reference/beta/api/schemaextension_post_schemaextensions.md)
- [schemaExtension abrufen](../api-reference/beta/api/schemaextension_get.md)
- [schemaExtension aktualisieren](../api-reference/beta/api/schemaextension_update.md)
- [schemaExtension löschen](../api-reference/beta/api/schemaextension_delete.md)
