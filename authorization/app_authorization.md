...
> **Hinweis:** Die `302 Found`-Antwort oben unterscheidet sich von der `302 Found`-Antwort, die Sie erhalten, wenn Sie den Anmeldevorgang für die `https://login.windows.net/{tenantId}/oauth2/authorize?...`-URL starten. Im letztgenannten Fall leitet die `302 Found`-Antwort Ihre Anforderung an `login.microsoftonline.com` um.
